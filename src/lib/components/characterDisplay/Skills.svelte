...
<div>
    {#each attributes as attr}
        <div class="py-1">
            <button class="text-4xl" on:click={() => showSkills(attr)}>
                ...
                <span
                    >{presentAttribute(
                        $selectedCharacter.attributes[attr]
                    )}</span
                >
            </button>
    ...
    {/each}
    <div>
        <button class="text-4xl" on:click={() => showSkills("tool")}>
            tools
        </button>
        {#each $selectedCharacter.tools as tool}
            {#if selectedAttr == "tool"}
                <div>
                    <p>
                        <span
                            class:text-blue-500={!tool.expertise}
                            class:text-yellow-500={tool.expertise}
                        >
