...
{#each $characterRepository.current.attacks as attack, index}
    <Collapsible>
        <div slot="title" class="flex flex-col text-left my-2 w-full">
            <Editable>
                <input slot=editing id={`attack-${index}-name`} type="text" class="input w-full" bind:value={attack.name}>
            ...
            <span class="text-xl">
                {#if attackHitBonus(attack) > 0}
                    ...
                    {attack.damage}{#if attack.attribute}<SignedNumber number={$attributesModifiers[attack.attribute]} />{/if}
                {:else}
                    {attack.damage}
                {/if}
        ...
        <div slot="body">
            <Editable>
                ...
                <div class="py-4">
                    ...
                    <input type="text" id={`attack-${index}-damageType`} class="input w-full" bind:value={attack.damageType}>
                </div>
                ...
                <div class="py-4">
                    ...
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <div>
                                <input id={`attack-${index}-hitBonus`} type="number" class="input w-12 text-center" bind:value={attack.hitBonus}>
            ...
            <div class="py-4">
                <Editable>
                    ...
                    <textarea slot=editing id={`attack-${index}-notes`} class="input w-full" bind:value={attack.notes} placeholder={t('display.missingNotes')}/>
                </Editable>
            </div>
        </div>
    </Collapsible>
