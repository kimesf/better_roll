...
{#each $characterRepository.current.features as feature, index}
    <Collapsible>
        <div slot="title" class="text-xl my-2 w-full">
            <Editable>
                <input slot=editing id={`feature-${index}-name`} type="text" class="input w-full" bind:value={feature.name}>
        ...
        <div slot="body" class="py-4">
            <Editable>
                <textarea slot=editing id={`feature-${index}-notes`} class="input w-full" bind:value={feature.notes} placeholder={i18n.t('display.missingNotes')}/>
