<script lang="ts">
  ...
  const openMenu = (key) => (menuOpened = key)
  const closeMenu = () => (menuOpened = null)
</script>
  ...
  {#each menuKeys as key}
    ...
    {#if menuOpened == key}
      <div class="bg-neutral-900 absolute min-h-screen w-screen flex flex-col justify-between">
        ...
        <button on:click={() => closeMenu()} class="fixed bottom-0 w-screen bg-neutral-800 p-4 text-2xl text-center">
          GO BACK
        </button>
      </div>
    {/if}
        ...
        <nav>
          <ul>
            {#each menuKeys as key}
              <li class="flex">
                <button
                  on:click={() => openMenu(key)}
                  class="p-1 text-2xl w-full"
                >
                  {i18n.t(`display.${key}`)}
                </button>
              </li>
